...
<div class="w-full text-2xl lg:text-base">
  ...
  <h2>Created on <%=Moment.moment(thread.getEntry().getCreationDate()).format("dd/MM/YY hh:mm")%> by <a class="text-java-blue" href="/user/<%=thread.getEntry().getAuthor().getUsername()%>"><%=thread.getEntry().getAuthor().getUsername()%></a></h2>
  <a class="w-full" href="/thread/<%=thread.getId()%>"><%=thread.getEntry().getContentSummary(20)%></a>
</div>